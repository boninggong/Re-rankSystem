...
# Converts all re-rank and initial evaluation results to LaTeX tables
...
for dimension in dimensions:
    print(f'\\FloatBarrier\n{{\\Large\\textbf{{{dimension} dimension based re-rankings}}}}')
    for algo in algos:
        algo_d = algo
        if algo == 'CAMF_ICS':
            algo_d = "CAMF-ICS"
        for m in metrics_sizes:
            for t in test_items:
                if m != 'all':
                    algo_glob_p = pd.read_csv(f"{path}{t}\\global-{algo}-{metrics[0]}-{dimension}.csv", delimiter=',')
                    algo_glob_map = pd.read_csv(f"{path}{t}\\global-{algo}-{metrics[1]}-{dimension}.csv", delimiter=',')
                    algo_pers_p = pd.read_csv(f"{path}{t}\\personal-{algo}-{metrics[0]}-{dimension}.csv", delimiter=',')
                    algo_pers_map = pd.read_csv(f"{path}{t}\\personal-{algo}-{metrics[1]}-{dimension}.csv", delimiter=',')
                    ...
                    output = f"\\begin{{table}}[ht]\n\\captionsetup{{justification=centering}} \\scalebox{{0.85}}{{" \
                             ...
                             f"\t \\\\ \\hline\n" \
                             f"\\end{{tabular}}}}\n\\caption*{{Re-rank results for the \\textbf{{{algo_d}}} initial recommendation " \
                             ...
                             f"label{{tab:res-{algo_d}-{t}-{m}}}\n\\end{{table}}"
                else:
                    algo_glob_map = pd.read_csv(f"{path}{t}\\global-{algo}-{metrics[1]}-{dimension}.csv",
                                                delimiter=',')
                    algo_pers_map = pd.read_csv(f"{path}{t}\\personal-{algo}-{metrics[1]}-{dimension}.csv",
                                                delimiter=',')
